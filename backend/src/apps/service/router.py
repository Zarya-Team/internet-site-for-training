...
@router.get("/countries", response_model=List[CountriesSchema], tags=["Катигории для фильтров"], summary="Получение всех стран")
...
@router.get("/countries_recipes", response_model=List[RecipesSchema], tags=["Получить данные по фильтру"], summary="Получение рецептов по стране")
async def countries_recipes_get_all(
    ...
    query = await sesion.execute(
        select(Recipes)
        .filter(Recipes.countries_id == countries_id)
        .offset(offset)
        .limit(per_page)
    )
    return [c[0].to_read_model() for c in query]

@router.get("/categories_recipes", response_model=List[RecipesSchema], tags=["Получить данные по фильтру"], summary="Получение рецептов по категориям")
async def categories_recipes_get_all(
    ...
    query = await sesion.execute(
        select(Recipes)
        .filter(Recipes.categories_id == categories_id)
        .offset(offset)
        .limit(per_page)
    )
    return [c[0].to_read_model() for c in query]

@router.get("/categories", response_model=List[CategoriesSchema], tags=["Катигории для фильтров"], summary="Получение всех категорий")
...
@router.get("/recipe", response_model=RecipeIngredientsSchema, tags=["Получить полные данные"],  summary="Получение всей информации о рецепте")
